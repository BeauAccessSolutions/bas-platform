...
| **Marketing site** | Company site (not a platform app) | Astro + Netlify | `Beaudoin0zach/bas-website` (private, pushed 2026-07-14) | — | 🟡 repo live; remade around the app portfolio |
...
| **Marketing site** | Astro static | **Netlify** | [`netlify.toml`](repos/marketing-site/netlify.toml) — build `dist`, SPA redirect, security headers | Netlify git deploy | ⬜ | 🟡 **repo pushed** (`Beaudoin0zach/bas-website`, private) · ⬜ Netlify site not yet connected (pending owner's Netlify re-auth) |
...
- 🟡 **Marketing site** — ✅ repo created + pushed as **`Beaudoin0zach/bas-website`** (private, 2026-07-14); the site was **remade around the app portfolio** (new `/apps` page + home repositioning + nav). ⬜ **Netlify connect pending** — owner is re-authing the Netlify CLI to a non-`langworthywatch` account, then: `netlify init` (or Netlify UI → Import from Git → `bas-website`) with build `npm run build` / publish `dist` (already in `netlify.toml`); a **private** repo needs the Netlify GitHub app authorized on it. Minor: `netlify.toml` has an SPA `/* → /index.html` catch-all — fine, but for a static Astro site consider pointing 404s at a real 404 page instead.
