...
- 🟡 **`/bas-design-review` sweep (2026-07-14).** Reviewed to the WCAG 2.2 AA bar. ✅ **Done:** Benefits Navigator, CIT, **Keycloak login theme** (passed static layer; 2 CSS fixes applied — 24px checkbox + guaranteed 16px input; open: verify the inherited `keycloak.v2` error path announces/moves focus), **KindredAccess** (non-chat surfaces — found + fixed a **dark-theme contrast blocker** on the signup password meter/requirements, plus autocomplete/dark-theme/live-region should-fixes; branch `a11y/design-review-fixes` pushed, ⬜ not merged), **Access Atlas** (passed; fixed the **form-input-loss on validation** via a zero-JS cookie echo + field-level errors + up-front size limit; branch `a11y/form-input-preservation` pushed, ⬜ not merged — merging touches `deploy_on_push` main). ⬜ **Still owed:** page-repair (whole extension), a11y-probe, marketing site. Minor deferred: KA password-match input *border* color (`signup.html:406`) is theme-fixed hex (reinforced by text, so not color-alone).
