...
- **Mobile source backup:** `bas-apps` (Baseline + shared packages), `kindredaccess-ios`, and `bas-frontend` are all now pushed to private `Beaudoin0zach/*` repos. Only `access-atlas-mobile` remains local (it has uncommitted work — kept, not dropped). See §6.
...
- ✅ **Mobile app source backup — done (2026-07-14).** Pushed to private `Beaudoin0zach/*` repos: **`bas-apps`** (the **Baseline**/CIT native Expo app + shared `ui`/`auth`/`tokens`/`i18n`/`api` packages), **`kindredaccess-ios`** (the KA wrapper), and **`bas-frontend`**. The Access Atlas + KindredAccess *web* wrapper configs already live inside their pushed web repos. ⏳ **`access-atlas-mobile` kept local, NOT dropped** — it turned out to have real uncommitted work (App.js +101 lines, a new `eas.json`), so it's not the throwaway it looked like; decide whether to fold that work into the canonical `access-directory` Capacitor wrapper or back it up separately. Note: `kindredaccess-ios` has uncommitted `appstore-screenshots/` (untracked, not pushed).
