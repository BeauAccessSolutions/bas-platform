...
- 🟡 **CIT numeric-PHI leak found in passing (2026-07-18)** — CIT's PHI drift test parses `schema.prisma` but collects only `String` columns, so `bpSystolic` / `bpDiastolic` / `heartRate` / `temperature` were unclassified in `PHI_FIELDS` and passed unredacted into stringified Prisma errors. Fix open as [Chronic-Illness-Tracker#41](https://github.com/Beaudoin0zach/Chronic-Illness-Tracker/pull/41). General lesson recorded: *a derived test still has the shape of its parser — check what your parser cannot see.*
