...
- **Mobile source backup — partly done:** `bas-apps` (Baseline app + shared packages) is now pushed to `Beaudoin0zach/bas-apps` (private). Still unbacked (no git remote): `kindredaccess-ios`, `bas-frontend`, abandoned `access-atlas-mobile`. See §6.
...
- 🟡 **Mobile app source backup — partly done (2026-07-14).** `bas-apps` — the **Baseline**/CIT native Expo app **plus** the shared `ui`/`auth`/`tokens`/`i18n`/`api` packages — is now pushed to **`Beaudoin0zach/bas-apps`** (private). ⚠ Still **unbacked (no git remote, local-only)**: `kindredaccess-ios` (the KA wrapper), `bas-frontend`, and the abandoned `access-atlas-mobile` — decide names/visibility (like the a11y-probe call) and push, or drop `access-atlas-mobile`. The Access Atlas + KindredAccess *web* wrapper configs live inside their already-pushed web repos, so those are safe.
